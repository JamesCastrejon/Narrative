...
class Page: UIViewController {
    ...
    @IBAction func menuVisibility(_ sender: Any) {
        ...
        if !buttonExit.isEnabled {
            ...
            bManager.enable(buttonDeletePage)
    ...
    @IBAction func changeBackgroundImportImage(_ sender: Any) {
        picker.didFinishPicking { [unowned picker] items, _ in
            if let photo = items.singlePhoto {
                self.viewPage.backgroundColor = UIColor.white
    ...
    @IBAction func changePageFormat(_ sender: Any) {
        ...
        let width = viewPage.frame.width
        let height = viewPage.frame.height
    ...
    @IBAction func deletePage(_ sender: Any) {
        if Book.orderedViewControllers.count > 3 && self.restorationIdentifier! != "FrontCover" && self.restorationIdentifier! != "BackCover" {
            self.animator.pop(buttonDeletePage, 0.5, 0.2)
    ...
    func colorPickerView(_ colorPickerView: ColorPickerView, didSelectItemAt indexPath: IndexPath) {
        ...
        if !buttonPalette.isHidden {
            ...
            if previousColor != colorPickerView.colors[indexPath.row] {
                ...
                viewPage.backgroundColor = colorPickerView.colors[indexPath.row]
            }
            else {
...
extension Page: ChromaColorPickerDelegate {
    func colorPickerDidChooseColor(_ colorPicker: ChromaColorPicker, color: UIColor) {
        if !buttonProPalette.isHidden {
            imageBackground.image = nil
            previousColor = color
            viewPage.backgroundColor = color
            bManager.enable(buttonBackground)
            bManager.enable(buttonPalette)
            bManager.enable(buttonImport)
...
extension Page: UITextFieldDelegate {
